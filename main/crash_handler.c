...
void check_and_report_crash(void)
{
    ...
    while (!wifi_is_connected() && retries-- > 0) {
        ESP_LOGI(TAG, "WiFi connection attempt %d/30", 30 - retries);
        vTaskDelay(pdMS_TO_TICKS(3000));
    }

    if (wifi_is_connected()) {
        ESP_LOGI(TAG, "Wi-Fi connected. Sending %s log report...", prefix);
        esp_err_t send_result = send_status_update_with_status(status_message, CONFIG_SENSOR_ID, CONFIG_BEARER_TOKEN);
